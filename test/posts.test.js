...
describe("posts — restored entries", () => {
  it("keeps the original numeric slugs", () => {
    expect(published.map((p) => p.slug).sort()).toEqual(["7", "8"]);
  });

  it("uses the Typecho created date", () => {
    const byslug = Object.fromEntries(published.map((p) => [p.slug, p]));
    expect(byslug["7"].date).toBe("2026-01-29");
    expect(byslug["8"].date).toBe("2026-02-02");
  });

  it("keeps the original titles", () => {
    const byslug = Object.fromEntries(published.map((p) => [p.slug, p]));
    expect(byslug["7"].title).toBe("对抗无聊");
    expect(byslug["8"].title).toBe("1");
  });
  ...
  it("keeps the body text intact (spot checks against the source)", () => {
    ...
    expect(byslug["7"].body).toContain("多接触日光，少接触能够快速大量产生多巴胺的活动");
    expect(byslug["7"].body).toContain("[IMDB TOP 250](http://www.imdb.com/chart/top)开始看电影");
    expect(byslug["7"].body.split("\n")).toHaveLength(8);
    expect(byslug["8"].body).toContain("1.  **1.30：** 2.74km");
    expect(byslug["8"].body).toContain("23. **3.10：** 3.17km");
    expect(byslug["8"].body.split("\n").filter(Boolean)).toHaveLength(23);
  });
  ...
  it("exposes exactly the expected slug set", () => {
    ...
    expect(posts.map((p) => p.slug).sort()).toEqual(["7", "8", "building-a-digital-presence"]);
  });
});
...
describe("posts — summaries", () => {
  ...
  const restored = posts.filter((p) => p.slug === "7" || p.slug === "8");
