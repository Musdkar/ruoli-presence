// Official brand glyphs for the Connect rail.
//
// Paths are taken verbatim from the `simple-icons` package (the canonical
// source for brand marks, https://simpleicons.org) rather than hand-drawn.
// Regenerate with:
//
//   node -e "const s=require('simple-icons');
//     for (const k of ['siGithub','siVrchat','siTelegram'])
//       console.log(k, JSON.stringify(s[k].path))"
//
// They are copied in rather than imported at runtime: importing the package
// pulls a 3400-entry index through the bundler, and even though it tree-shakes,
// the four literals cost ~1 kB against several kB of module boundary and build
// time. The dev dependency stays so the source is verifiable and refreshable.
//
// Each mark is a 24x24 viewBox path filled with currentColor, so the existing
// .socials hover and .disabled rules apply with no extra variants.

// GitHub, VRChat and Telegram are official Simple Icons paths.
//
// The contact tile is the exception: it opens the in-app /email route, not a
// mail provider, and Simple Icons ships no generic envelope (every mail glyph
// there is a specific brand - Gmail, Proton, Mail.ru). A neutral envelope is
// drawn by hand for that one tile rather than mislabelling the link with
// someone else's logo.
const PATHS = {
  github:
