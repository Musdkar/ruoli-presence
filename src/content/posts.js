export const posts = [
  {
    slug: "building-a-digital-presence",
    title: "Building a digital presence",
    date: "2026-09-16",
    summary:
      "Notes on making a personal homepage feel alive without rebuilding every collector, API and renderer from scratch.",
    tags: ["web", "presence", "design"],
    published: false,
    body: `# Building a digital presence

This site is designed around one rule: **the page should feel like a place, not a dashboard**.

The visual layer is custom, but the plumbing should use mature tools whenever possible. Discord presence comes from Lanyard, software activity comes from ActivityWatch, keyboard statistics come from WhatPulse, fitness data comes from Health Auto Export, and photo layout comes from React Photo Album.

The interesting part is not rebuilding those collectors. It is deciding what deserves to be visible, how much context to reveal, and how all of it should fit together without turning the page into an admin panel.

## Current structure

- Home — a live overview
- Photo — VRChat screenshots and visual memories
- Blog — longer notes
- Uses — the software I actually use

This post is a draft and is intentionally not published yet.`,
  },
  {
    slug: "1",
    title: "对抗无聊",
    date: "2026-01-29",
    summary:
      "多接触日光，少接触能够快速大量产生多巴胺的活动 每天接触10-30min的日光，可以每天早起去跑步 褪黑素的长期使用会抑制多巴胺水平 芝麻、奶酪、牛肉、鱼肉、坚…",
    tags: [],
    published: true,
    body: `- 多接触日光，少接触能够快速大量产生多巴胺的活动
- 每天接触10-30min的日光，可以每天早起去跑步
- 褪黑素的长期使用会抑制多巴胺水平
- 芝麻、奶酪、牛肉、鱼肉、坚果 补充络氨酸，帮助多巴胺生成
- 中午12点之前可以适当地喝一些咖啡，让多巴胺回路更加敏感
- 在无聊的时候做一些主观上更难受的事，从而快速恢复状态
- 在重复枯燥的工作中找到新鲜感
- 学习另外的一门外语或者从[IMDB TOP 250](http://www.imdb.com/chart/top)开始看电影`,
  },
  {
    slug: "2",
    title: "1",
    date: "2026-02-02",
    summary:
      "1.30： 2.74km 1.31： 2.88km 2.01： 2.92km 2.02： 2.90km 2.03： 2.85km 2.05： 3.03km 2.…",
    tags: [],
    published: true,
    body: `1.  **1.30：** 2.74km
2.  **1.31：** 2.88km
3.  **2.01：** 2.92km
4.  **2.02：** 2.90km
5.  **2.03：** 2.85km
6.  **2.05：** 3.03km
7.  **2.07：** 2.67km
8.  **2.08：** 2.83km
9.  **2.09：** 2.94km
10. **2.11：** 2.98km
11. **2.12：** 2.77km
12. **2.15：** 2.17km
13. **2.17：** 2.64km
14. **2.20：** 2.96km
15. **2.23：** 2.85km
16. **2.24：** 2.82km
17. **3.02：** 2.42km
18. **3.03：** 2.70km
19. **3.04：** 2.97km
20. **3.05：** 3.59km
21. **3.06：** 2.98km
22. **3.09：** 2.74km
23. **3.10：** 3.17km`,
  },
];
